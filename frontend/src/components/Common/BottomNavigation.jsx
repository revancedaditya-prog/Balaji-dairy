...
const BottomNavigation = ({ activeTab, setActiveTab }) => {
  ...
  const items = [
    ...
    },
    {
      ...
      label: 'Sales',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="8" cy="21" r="1"/><circle cx="19" cy="21" r="1"/><path d="M2.05 2.05h2l2.66 12.42a2 2 0 0 0 2 1.58h9.78a2 2 0 0 0 1.95-1.57L22 7H5.12"/></svg>
      )
    },
    {
      id: 'reports',
    ...
    <nav className="bottom-nav mobile-only">
      {filteredItems.map(item => {
        const isActive = activeTab === item.id ||
          (item.id === 'settings' && ['users', 'payments'].includes(activeTab));
